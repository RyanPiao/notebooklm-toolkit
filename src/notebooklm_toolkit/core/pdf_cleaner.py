...
def _build_watermark_mask(roi_bgr, cfg):
    h, w = roi_bgr.shape[:2]
    if h < 5 or w < 5:
        return None
    ksize = max(11, min(31, (min(h, w) // 6) | 1))
    background = cv2.medianBlur(roi_bgr, ksize)
    diff_gray = cv2.cvtColor(cv2.absdiff(roi_bgr, background), cv2.COLOR_BGR2GRAY)
    _, binary = cv2.threshold(diff_gray, cfg.pixel_threshold, 255, cv2.THRESH_BINARY)
    ...
    for i in range(1, num_labels):
        ...
        if area < cfg.min_component_area:
            continue
        if cx + cw / 2 < w * 0.5 or cy + ch / 2 < h * 0.5:
            continue
...
def _find_watermark_rect_text(page, cfg):
    ...
    return fitz.Rect(
        max(0, wm_rect.x0 - pad), max(0, wm_rect.y0 - pad),
        min(w, wm_rect.x1 + pad), min(h, wm_rect.y1 + pad),
    )


def _patch_pdf_rect(page, rect, cfg, precision=True):
    ...
    if precision:
        ...
        background = cv2.medianBlur(roi_bgr, ksize)
        diff_gray = cv2.cvtColor(cv2.absdiff(roi_bgr, background), cv2.COLOR_BGR2GRAY)
        _, mask = cv2.threshold(diff_gray, cfg.pixel_threshold, 255, cv2.THRESH_BINARY)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        mask = cv2.dilate(mask, kernel, iterations=3)
...
def render_page_to_png(page, output_path, cfg):
    ...
    if cfg.supersample > 1:
        pil_image = pil_image.resize((target_width, target_height), resample=Image.LANCZOS)
...
def build_task_list(pdf_paths, output_dir, cfg):
    tasks = []
...
def run_batch(pdf_paths, output_dir, cfg, progress_callback=None, cancel_check=None):
